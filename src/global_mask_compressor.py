    ...
    try:
        ...
        for token_idx in range(chunk_length):
            ...
            if args.encoding == "AC":
                ...
                probs_cpu = probs_values.to(torch.float32).numpy()
                ...
                target_idx = torch.tensor(actual_next_tokens, device=device, dtype=torch.long)
                batch_idx = torch.arange(logits.shape[0], device=device)
                target_logits = logits[batch_idx, target_idx].unsqueeze(1)
                ranks_0 = (logits > target_logits).sum(dim=1)
                ranks = ranks_0.cpu().tolist()
                for idx in range(args.batch_size):
                    if not valid_mask[idx]:
                        continue
                    rank_list.append(ranks[idx])
            else:
