...
def run_experiment(args):
    ...
    batch_size, context_length, gpu_id = args
    env = os.environ.copy()
    ...
    cmd = [
        ...
        "--first_n_tokens", str(NUM_TOKENS),
        "--use_kv_cache",
    ]
    print(f"[GPU {gpu_id}] Starting batch_size={batch_size}, context_length={context_length}")
    result = subprocess.run(cmd, env=env, capture_output=True, text=True)
    if result.returncode != 0:
        print(f"[GPU {gpu_id}] FAILED batch_size={batch_size}, context_length={context_length}\n{result.stderr}")
    else:
        print(f"[GPU {gpu_id}] Done batch_size={batch_size}, context_length={context_length}")
...
if __name__ == "__main__":
    ...
    print(f"Detected {num_gpus} GPU(s). Running one task per GPU.")
    ...
    # Distribute experiments across GPUs (round-robin assignment)
    tasks = [
        (bs, cl, idx % num_gpus)
        for idx, (bs, cl) in enumerate(experiments)
    ]

    # Run with at most num_gpus concurrent workers (one task per GPU)
    with ProcessPoolExecutor(max_workers=num_gpus) as executor:
        futures = {executor.submit(run_experiment, t): t for t in tasks}
        for future in as_completed(futures):
            bs, cl, gpu = futures[future]
            try:
                rc = future.result()
                if rc != 0:
                    print(f"Experiment batch_size={bs}, context_length={cl} on GPU {gpu} exited with code {rc}")
            except Exception as e:
                print(f"Experiment batch_size={bs}, context_length={cl} on GPU {gpu} raised: {e}")
