...
Reusable compression implementations remain in `src/`; paper-specific
commands live in `experiments/`.
...
```bash
HF_HUB_OFFLINE=1 bash research/papers/neurips_2026/experiments/run_all.sh fertility
HF_HUB_OFFLINE=1 bash research/papers/neurips_2026/experiments/run_all.sh attacks
```

The `attacks` stage generates the 10,000-token text8 and random-printable
controls with the same source-size budget as the adversarial rows.
...
without executing it. The optional `search` stage in `paper_evaluation.sh` is not used by
...
HF_HUB_OFFLINE=1 .venv/bin/python \
  research/papers/neurips_2026/evaluation/finalize_qwen_bars.py
HF_HUB_OFFLINE=1 .venv/bin/python \
  research/papers/neurips_2026/evaluation/quantization_sensitivity.py
.venv/bin/python research/papers/neurips_2026/evaluation/plot_prediction_difficulty.py
HF_HUB_OFFLINE=1 \
FSST_EXECUTABLE=/tmp/compression-economics-fsst/build/fsst \
  .venv/bin/jupyter nbconvert \
  --execute --to notebook --inplace \
  research/papers/neurips_2026/evaluation/crucial_figures.ipynb
```
...
```bash
cd research/papers/neurips_2026/manuscript
pdflatex main.tex
bibtex main
pdflatex main.tex
pdflatex main.tex
