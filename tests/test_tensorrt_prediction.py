...
def test_tensorrt_init_uses_main_cache_dir(monkeypatch):
    """TensorRT startup should use the same .cache behavior as the main branch."""
    from src.tensorrt_prediction import TensorRTTokenPredictor
    ...
    def fake_estimate(self, model_name):
        estimated.append(model_name)
        self.base_params = 0
    ...
    assert tokenizer_calls == [("Qwen/Qwen2.5-0.5B", {"cache_dir": ".cache"})]
    assert estimated == ["Qwen/Qwen2.5-0.5B"]
